...
module.exports = {
    mode: 'development',
    entry: {
        'proj1': path.resolve(__dirname, './src/index.js'),
        'proj1-entry': path.resolve(__dirname, './src/app.js')
    },
    output: {
        publicPath: process.env.NODE_ENV === 'development' ? '/dist/' : 'http://localhost:3000/',
        filename: process.env.NODE_ENV === 'development' ? 'proj1-[name].js' : 'proj1-[name].[chunkhash].js',
                            ...
                            plugins: [
                                ...
                                postcss.plugin('namespace', () => css =>
                                    ...
                                    if (rule.parent && rule.parent.type === 'atrule' && rule.parent.name !== 'media') return;
                                        rule.selectors = rule.selectors.map(s => `.namespace-project1 ${s === 'body' ? '' : s}`);
                                    })
                                )()
                            ],
                            exec: true } 
